...
3. `askChatbot(service, brains, turns)` is the Hive Mind loop over live brains: one system prompt rendering each brain's model index under a handle (`slugified kb_brain name + 4 hex of id`), page keys `handle/slug`, `readPagesTool` then a forced `chatbotAnswerTool` — the first call carries `tool_choice: any` (`mustUseTool` on `requestAnthropic`) so the model must read or answer through a tool and can never reply in prose, which the parser would turn into "I could not put an answer together" (seen 2026-09-04) — the shared answer tool plus two REQUIRED fields: `isKnowledgeGap` (boolean — true whenever the pages could not answer, in full or in part, including anything added from general knowledge and questions about the bot; false only for a pages-grounded answer, a greeting or thanks) and `missingKnowledge` (ONE owner-answerable question, empty when false). They are required because an optional field was silently skipped by Haiku (2026-09-04: "the knowledge base doesn't define RFQ" recorded no gap); when the flag is true but the wording is blank, the member's own question is recorded as the gap. Page reads group requested keys by handle and call `getBrainPagesBySlugs` per brain. The last 12 messages of the conversation are remembered, as elsewhere.

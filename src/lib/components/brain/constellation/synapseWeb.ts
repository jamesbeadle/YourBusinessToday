...
export function createSynapseWeb(model: ConstellationModel, bank: MaterialBank): SynapseWeb {
	...
	const dockRadiusFor = (slug: string) =>
		strandDockRadiusOf(slug, nucleusSlugs.has(slug) ? nucleusProportions : neuronProportions);
	...
	const strands = model.synapses.map((synapse) => {
		...
		const strand = createSynapseStrand(synapse, contextKey, material, dockRadiusFor);

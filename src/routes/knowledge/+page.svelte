	...
	<header class="flex flex-wrap items-end justify-between gap-4">
		<div class="flex flex-col gap-2">
			...
			<p class="max-w-prose text-chalk/70">
				A knowledge base holds two kinds of brains: domain brains that define abstract structure
				and rules, and instance brains that hold the data filling those structures.
			</p>
		</div>
		<button
			type="button"
	...
	{#if data.knowledgeBases.length === 0}
		...
		>
			<p class="max-w-prose text-chalk/60">
				Create your first knowledge base, then grow it with domain brains for structure and
				instance brains for the knowledge itself.
			</p>
			<button
				type="button"
				onclick={() => (isCreateModalOpen = true)}
...
<Modal title="New knowledge base" bind:isOpen={isCreateModalOpen}>
